 ...
 * y sólo lee.
 *
 * **Tiene que ser la URL del session pooler**, puerto 5432
 * (`postgres.<ref>@aws-N-us-west-2.pooler.supabase.com`): la conexión directa
 * de ese proyecto es sólo IPv6 y desde Railway da `timeout expired`.
 *
 * Se probó también su API de Supabase, que habría evitado la contraseña, pero
 * el PostgREST de ese proyecto está caído desde hace tiempo («Could not query
 * the database for the schema cache»); como Mercadito nunca lo usa, nadie se
 * había enterado.
 *
...
export async function cruzarMercadito(fecha: string): Promise<{ marcados: number; saltados: number; error?: string }> {
  ...
  const base = ayotl();
  // Decir cuál falta, y no un "sin configurar" a secas: el fallo típico es un
  // nombre mal escrito en Railway.
  if (!url || !base) {
    const faltan = [!url && "MERCADITO_DATABASE_URL", !base && "SUPABASE_URL/SUPABASE_SECRET_KEY"].filter(Boolean);
